...
func parseIP(ip string) []string {
	...
	// When scanning /8, only scan the gateway and random IP addresses to avoid scanning too many IPs.
	case strings.HasSuffix(ip, "/8"):
		return parseIP8(ip)
	// Parsing /24, /16, /8, /xxx, etc.
...
func IPRange(c *net.IPNet) string {
	...
	return fmt.Sprintf("%s-%s", start, end) // 192.168.1.0-192.168.255.255
}
